...
function extractHashtags($string) {
  ...
  $hashtags = array_slice(array_unique($matches[1]), 0, 4); // get the first 10 unique hashtags
  foreach ($hashtags as $value) {
    file_put_contents('C:/xampp/htdocs/newsurg/data/misc/hash.hash', $value . ',,!!,,', FILE_APPEND);
  }
}
    ...
    file_put_contents('C:/xampp/htdocs/newsurg/data/posts/posts/'.$fnametp,$fdatatp);
        ...
        if($ismedia==1) {
          ...
          file_put_contents('C:/xampp/htdocs/newsurg/data/posts/posts/'.$fnametp,$fdatatp);
              ...
              if($istext==1){
                if(!$ismedia==1) {
                  $url = "https://ptb.discord.com/api/webhooks/1102251034660839424/zsUDo0rtrMO-Eqg5BjJLyWZOc8P0z15OS4FI7YfqT_gwjPs9PypxfNJ0eu9ulGigXPGu";
